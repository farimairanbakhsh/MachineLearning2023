...
g_u(q)=0.6*sin(pi*u(q))+0.3*sin(3*pi*u(q))+0.1*sin(5*pi*u(q));
    ...
    y_hat(q+1)=0.3*y(q)+0.6*y(q-1)+f_hat(q);
        ...
        for l=1:M
            ...
            sigma(q+1,l)=sigma(q,l)-landa*((f_hat(q)-g_u(q))/b)*(g_bar(q,l)-f_hat(q))*z(l)*2*(x(q)-x_bar(q,l))/(sigma(q,l)^3);
        end
end
...
g_u(q)=0.6*sin(pi*u(q))+0.3*sin(3*pi*u(q))+0.1*sin(5*pi*u(q));
    ...
    b=b+z(l);a=a+g_bar(num_training,l)*z(l);
    end
f_hat(q)=a/b;
y(q+1)=0.3*y(q)+0.6*y(q-1)+g_u(q);
y_hat(q+1)=0.3*y(q)+0.6*y(q-1)+f_hat(q);
...
legend('output pf the plant','output of the identification model')
axis([0 701 -5 5]);
grid on
